...
The reference module selects a local origin without promoting that origin into
the origin of the universe. Its output is an addressed zero: a declared state,
resolution, duration, location, and scale tier. Zero Infinity supplies the
recursively available reference architecture; the local reference supplies the
measurement origin for this inquiry.
...
classification. The sign does not replace magnitude, unit, duration, or state.
...
- the active enclosure whose persistence is under study;
- an enclosing equilibrium projecting admissible conditions inward;
- enclosed states whose contributions maintain or alter the active state;
- parallel enclosures sharing boundaries or conditions.

A non-equilibrium steady state may be enclosed and maintained by a larger
steady-state equilibrium. Inventory turnover does not disprove persistence.
...
outer activity is recorded as emergence, contribution, feedback, or exchange.
...
uncertainty, and residual. Destruction is not inserted where a transition can
be observed. An unexplained residual remains an unresolved entry.
...
the intervention, prediction, competing result, falsification condition, and
route back to an actionable scale.

**Failure condition:** the intervention is selected before the active enclosure
and exchange ledger are complete.

## Star-Map Grammar

| Map object | Meaning |
|---|---|
| Star | observation, state, or measured departure |
| Constellation boundary | active causal enclosure |
| Inner orbit | enclosed state |
| Outer orbit | enclosing equilibrium |
| Neighboring constellation | parallel enclosure |
